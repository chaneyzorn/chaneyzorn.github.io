...
- `layouts/_partials/extend_footer.html` — 主题的自定义注入点：窄屏滚动方向感知的导航自动显隐 JS（配合 accent.css 的 `.scroll-down` 规则）、Mermaid 按需加载与点击放大模态层、正文图片点击放大模态层（两套模态层样式在 accent.css，Esc 关闭依赖模态层的 `tabindex="-1"`）
