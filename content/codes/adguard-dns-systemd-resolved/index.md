...
我使用的 DNS 服务是 [AdguardHome](https://github.com/AdguardTeam/AdGuardHome)，它默认监听在 `0.0.0.0:53/udp` 上，但是在使用了 systemd 全家桶的节点上，systemd-resolved 会监听在 `127.0.0.53:53/dup` 上，因此会导致端口冲突，Adguard [官方 wiki 给出的解决方式](https://github.com/AdguardTeam/AdGuardHome/wiki/Docker#resolved)是修改 resolved 的配置，停止监听 `127.0.0.53:53`。
...
除这些模块外，如果安装了 [Avahi](https://github.com/avahi)，也可以使用 [`nss-mdns`](https://github.com/avahi/nss-mdns) 模块，它会提供 mDNS 查询结果。当然在使用 systemd-resolved 之后就已经包含这个功能了。
